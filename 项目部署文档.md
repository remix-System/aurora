...
=======
   docker run --name nginx --restart=always -p 443:443 -p 80:80 -d -v /etc/nginx/nginx.conf:/etc/nginx/nginx.conf -v /etc/nginx/cert:/etc/nginx/cert -v /usr/local/vue:/usr/local/vue --privileged=true nginx
   ```
